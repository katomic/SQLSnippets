-- Get Datasets, datasources etc for your SSRS reports
...
Data AS (
    SELECT ds.ItemID,
        Name,
        Path,  
        LEFT( Path, CHARINDEX('/', Path, 2 ) -1 ) as Folder, 
        CHARINDEX('/', Path, 2 ) as Folderindex, 
        LocalDataSourceName,
        DataSetName,
...
SELECT * FROM Data
where ( LEFT(Path, Folderindex) = '/REPORTS_FOLDER1/' --## IF you have this FOLDER
ORDER BY  Name, CommandText, Path
